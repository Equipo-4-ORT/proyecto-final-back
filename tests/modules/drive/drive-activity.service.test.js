...
jest.mock('../../../src/shared/database/prisma', () => ({
  ...
  // persistDriveActivities resuelve el googleId del usuario para la atribución
  // por personName (docs fuera de "Mi unidad"). Por defecto, un id propio que no
  // coincide con el de "otros" colaboradores en los tests de descarte.
  user: {
    findUnique: jest.fn().mockResolvedValue({ googleId: 'self-account-id' }),
  },
...
describe('Drive Activity Service', () => {
  ...
  describe('getDriveActivitiesForDay', () => {
    test('Devuelve array vacío si la API no retorna actividades', async () => {
      ...
      const result = await getDriveActivitiesForDay(
        'token',
      ...
      mockActivityQuery.mockImplementation(({ requestBody }) => {
        ...
        if (!requestBody.pageToken) {
          return Promise.resolve({
            data: { activities: [page1], nextPageToken: 'token-pagina-2' },
          });
        }
  ...
  describe('buildWorkEstimates', () => {
    ...
    test('acción continua única: endTime = startMs + buffer de 5 min', () => {
      ...
      const byFile = new Map([
        ['doc1__edit', entry({ intervals: [{ startMs: tsMs, endMs: null }] })],
      ]);
      ...
      const byFile = new Map([
        [
          ...
          entry({
            ...
            mimeType: null,
            intervals: [
              { startMs: t2, endMs: null },
              { startMs: t1, endMs: null },
              { startMs: t3, endMs: null },
            ],
          }),
        ],
      ]);
      ...
      const byFile = new Map([
        [
          ...
          entry({
            mimeType: null,
            title: null,
            intervals: [{ startMs: tsMs, endMs: null }],
          }),
        ],
      ]);
    ...
    test('fileType es "file" para mimeTypes no nativos de Workspace (PDF, imagen, etc.)', () => {
      const byFile = new Map([
        ['pdf1__edit', entry({ fileId: 'pdf1', mimeType: 'application/pdf', title: null })],
      ]);
    ...
    test('fileType es "file" cuando mimeType es null', () => {
      const byFile = new Map([
        ['file1__edit', entry({ fileId: 'file1', mimeType: null, title: null })],
      ]);
      ...
      const byFile = new Map([
        ...
        [
          ...
          entry({
            ...
            mimeType: null,
            title: null,
            intervals: [{ startMs: t1 + 2000, endMs: null }],
          }),
        ],
      ]);
    ...
    test('el title de primer nivel antepone el verbo de la acción al nombre del archivo', () => {
      const tsMs = new Date('2026-05-26T10:00:00.000Z').getTime();
      const byFile = new Map([
        ['doc1__edit', entry({ title: 'Mi informe', intervals: [{ startMs: tsMs, endMs: null }] })],
      ]);
    ...
    test('title de primer nivel es solo el verbo cuando el archivo no tiene nombre', () => {
      const tsMs = new Date('2026-05-26T10:00:00.000Z').getTime();
      const byFile = new Map([
        ['doc1__edit', entry({ title: null, intervals: [{ startMs: tsMs, endMs: null }] })],
      ]);
  ...
  describe('persistDriveActivities', () => {
    ...
    test('Usa la ventana [startTime, endTime) recibida en el filtro de la API', async () => {
      ...
      expect(filter).toBe(
        'time >= "2026-05-26T03:00:00.000Z" AND time < "2026-05-27T03:00:00.000Z"',
      );
    });
    ...
    test('Persiste todas las acciones (rename, move, delete, comment) como registros separados', async () => {
      ...
      const actions = getUpsertedRecords()
    ...
    test('Archivo sin mimeType nativo (PDF): fileType="file"', async () => {
      const activity = {
        primaryActionDetail: { edit: {} },
        targets: [
          { driveItem: { name: 'items/pdf1', title: 'contrato.pdf', mimeType: 'application/pdf' } },
        ],
        actors: [CURRENT_USER],
        timestamp: '2026-05-26T10:00:00Z',
      };
    ...
    test('sanea + trunca el title del archivo (control chars y longitud) antes de persistir', async () => {
        ...
        targets: [
          {
            driveItem: {
              name: 'items/doc1',
              title: `Doc\x00\x07umento${'C'.repeat(5000)}`,
              mimeType: 'application/vnd.google-apps.document',
            },
          },
        ],
      ...
      // metadata.title guarda el mismo valor que el title de primer nivel (verbo + nombre).
      // eslint-disable-next-line no-control-regex
      expect(saved.metadata.title).not.toMatch(/[\x00-\x08\x0E-\x1F\x7F]/);
      expect(saved.metadata.title.includes('Documento')).toBe(true);
      // El título (incluyendo el prefijo de acción) no supera el tope visible.
      expect(saved.metadata.title.length).toBeLessThanOrEqual(210);
    });
  ...
  describe('summarizeDriveActivities', () => {
    ...
    test('Agrupa varias acciones del mismo archivo en una sola entrada', () => {
      const activities = [docActivity('edit'), docActivity('edit'), docActivity('comment')];
  ...
  describe('Escenarios de negocio', () => {
    ...
    describe('Solo comentarios', () => {
      ...
      test('Varios archivos con comentarios: cada uno se persiste', async () => {
        ...
        const fileIds = prisma.dailyActivity.upsert.mock.calls
          .map((c) => c[0].create.metadata.fileId)
          .sort();
            ...
            activities: [
              {
                ...
                targets: [
                  {
                    driveItem: { name: 'items/shared2', title: 'Archivo externo', mimeType: null },
                  },
                ],
                actors: [CURRENT_USER],
                timestamp: '2026-05-26T10:00:00.000Z',
        ...
        mockFilesGet
          .mockResolvedValueOnce({
            data: {
              name: 'Nombre fresco',
              mimeType: 'application/vnd.google-apps.document',
              webViewLink: 'https://docs.google.com/own1',
            },
          })
  ...
  describe('enrichDriveActivitySummary', () => {
      ...
      mockFilesGet
        ...
        .mockResolvedValueOnce({
          data: {
            name: 'Presupuesto',
            mimeType: 'application/vnd.google-apps.spreadsheet',
            webViewLink: 'https://sheets.google.com/sheet1',
          },
        });
    ...
    test('Sanea el nombre fresco del archivo antes de asignarlo como title', async () => {
      mockFilesGet.mockResolvedValue({
        data: {
          name: `Doc\x00\x07umento${'C'.repeat(5000)}`,
          mimeType: 'application/vnd.google-apps.document',
          webViewLink: 'https://x',
        },
      });
  ...
  describe('Cobertura fuera de Mi unidad y atribución por actor', () => {
    ...
    test('CA-02: persiste actividad propia en una Unidad compartida (ancestorName)', async () => {
      ...
      mockActivityQuery.mockImplementation(
        scopedActivityQuery([
    ...
    test('CA-01: persiste actividad propia sobre un documento "Compartido conmigo" (itemName)', async () => {
      mockFilesList.mockResolvedValue({
        data: { files: [{ id: 'extFile', mimeType: 'application/vnd.google-apps.document' }] },
      });
      mockActivityQuery.mockImplementation(
        scopedActivityQuery([
    ...
    test('CA-01b: persiste actividad propia en doc compartido cuando la API NO marca isCurrentUser pero el personName coincide (fix F-DRIVE-02)', async () => {
      // En docs fuera de "Mi unidad" la Activity API devuelve el actor como
      // knownUser con personName pero SIN isCurrentUser. La atribución debe
      // reconocerlo por personName = people/{googleId} (mock: 'self-account-id').
      const sharedActivityNoFlag = {
        primaryActionDetail: { edit: {} },
        targets: [
          {
            driveItem: {
        ...
        actors: [{ user: { knownUser: { personName: 'people/self-account-id' } } }],
        timestamp: '2026-05-26T10:00:00.000Z',
      };
      mockFilesList.mockResolvedValue({
        data: { files: [{ id: 'extFile', mimeType: 'application/vnd.google-apps.document' }] },
      ...
      mockActivityQuery.mockImplementation(
        scopedActivityQuery([
          { key: 'itemName', value: 'items/extFile', activity: sharedActivityNoFlag },
        ]),
      );
    ...
    test('CA-04b: descarta actividad en doc compartido cuando el personName NO es el del usuario (otro colaborador, sin isCurrentUser)', async () => {
      const otherCollaborator = {
        ...
        actors: [{ user: { knownUser: { personName: 'people/otro-colaborador' } } }],
        timestamp: '2026-05-26T10:00:00.000Z',
      };
      mockFilesList.mockResolvedValue({
        data: { files: [{ id: 'extFile', mimeType: 'application/vnd.google-apps.document' }] },
      ...
      mockActivityQuery.mockImplementation(
        scopedActivityQuery([
          { key: 'itemName', value: 'items/extFile', activity: otherCollaborator },
        ]),
      );
    ...
    describe('isCurrentUserActivity', () => {
      ...
      test('false para otros usuarios, sistema, anónimos o sin actor', () => {
        expect(
          isCurrentUserActivity({ actors: [{ user: { knownUser: { isCurrentUser: false } } }] }),
        ).toBe(false);
      ...
      test('true por personName cuando la API no marca isCurrentUser (docs fuera de Mi unidad)', () => {
        const selfNames = new Set(['people/self-account-id']);
        const sharedActivity = {
          actors: [{ user: { knownUser: { personName: 'people/self-account-id' } } }],
        };
        // Sin el set de identidades propias, no se puede atribuir.
        expect(isCurrentUserActivity(sharedActivity)).toBe(false);
        // Con el set, se reconoce por personName.
        expect(isCurrentUserActivity(sharedActivity, selfNames)).toBe(true);
      });

      test('false cuando el personName es de otro colaborador, aunque se pase el set propio', () => {
        const selfNames = new Set(['people/self-account-id']);
        expect(
          isCurrentUserActivity(
            { actors: [{ user: { knownUser: { personName: 'people/otro' } } }] },
            selfNames,
          ),
        ).toBe(false);
      });
